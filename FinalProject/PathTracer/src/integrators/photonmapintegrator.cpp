...
inline float gFilterPavicic90(const float rsquared) {
...
//                        || isect.bsdf->BxDFsMatchingFlags( (BxDFType)(BSDF_REFLECTION | BSDF_SPECULAR) ) > 0) ? true : false;
        ...
        Color3f colordirectsample = isect.bsdf->Sample_f(woW,&wiWdirectsample,sampler->Get2D(),&pdfdirectsample);
        ...
        Vector3f wiWbxdf;
        float pdfbxdf;
        Color3f colorbxdf = isect.bsdf->Sample_f(woW,&wiWbxdf,sampler->Get2D(),&pdfbxdf);
...
//          || isect.bsdf->BxDFsMatchingFlags( (BxDFType)(BSDF_REFLECTION   | BSDF_SPECULAR)) > 0) {
    while(   isect.bsdf->BxDFsMatchingFlags( (BxDFType)(BSDF_SPECULAR)) > 0) {
        ...
        Vector3f woW;
        float pdf = 1.f;
        Color3f color = isect.bsdf->Sample_f(wiW,&woW,xi,&pdf);
...
//        Vector3f woWdirectgwintegration = -theincray.direction;
        ...
        Vector3f wiWdirectsample(0.f);
        float pdfdirectsample;
        Color3f colordirectsample = sourceisect.bsdf->Sample_f(woWdirectgwintegration,&wiWdirectsample,sampler->Get2D(),&pdfdirectsample);
        if(glm::length(colordirectsample) > 0 && pdfdirectsample > 0) {
        ...
        for(int i = 0; i < INDIRECTRAYS; ++i) {
            Color3f indthroughput = throughput;
            Color3f pickupfromaccumcolor = accumcolor + totalcausticphotonsum;
            Color3f colorbxdf = sourceisect.bsdf->Sample_f(woW,&wiWbxdf,sampler->Get2D(),&pdfbxdf);
...
//        Color3f colorbxdf = sourceisectIND.bsdf->Sample_f(woW,&wiWbxdf,sampler->Get2D(),sampler->Get1D(),&pdfbxdf);
        ...
        Color3f colorbxdf = sourceisectIND.bsdf->Sample_f(woWindirectgwintegration,&wiWbxdf,sampler->Get2D(),&pdfbxdf);
