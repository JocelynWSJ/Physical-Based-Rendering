...
public:
    SpotLight(const Transform &t, const Color3f& Le, Float totalWidth, Float falloffStart)
        : Light(t), emittedLight(Le), pLight(t.position()), cosTotalWidth(std::cos(glm::radians(totalWidth))),
    ...
    virtual float getProxyWattage() const;
    virtual Color3f getEmittedLight() const;
