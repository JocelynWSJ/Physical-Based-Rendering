...
#include <integrators/photonmapintegrator.h>
#include <QDateTime>
...
void MyGL::RenderScene()
{
    ...
    if(makePM && !scene.pm)
    {
        QTime timer = QTime();
        timer.start();
        scene.pm = new PhotonMap(&scene);
        int elapsed = timer.elapsed();
        std::cout << "\nPhotonMap execution time(ms): " << elapsed << "\n";
    }
    else if(makePM && scene.pm)
    {
        scene.clearPM();
        scene.pm = new PhotonMap(&scene);
    }
    ...
    for(int X = 0; X < nTiles.x; X++)
    {
        for(int Y = 0; Y < nTiles.y; Y++)
        {
            ...
            case INDIRECT_LIGHTING:
                rt = new PhotonMapIntegrator(tileBounds, &scene, sampler->Clone(seed), recursionLimit);
                break;
            case FULL_LIGHTING:
